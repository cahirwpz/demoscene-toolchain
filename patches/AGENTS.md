...
Quilt stack. `series` = apply order; quilt runs from repo root, `-p1` strips
the leading `demoscene-toolchain/`. `.pc/` tracks applied state.
...
Add or edit a patch (let quilt make the diff — don't hand-write):
  ...
  <prepend the prose header to the .diff>
Verify the stack: `quilt pop -a && quilt push -a`
(FAILED/fuzz = bad; context "offset N lines" = fine).
